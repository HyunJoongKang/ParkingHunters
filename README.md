...
3. `DAEGU_PARKING_CONGESTION_KEY`(실시간 혼잡도)는 IP 화이트리스트 방식이라, 국내 IP로
   옮겨도 Cloudtype이 사용하는 아웃바운드 IP를 대구시 쪽에 새로 등록해야 401이 나지
   않는다. Cloudtype에 고정 아웃바운드 IP가 없다면 기존처럼 `FIXIE_URL` 프록시가
   계속 필요할 수 있다 — 자세한 배경은 `app/lib/parkingApi.ts` 상단 주석 참고.
