...
// 대구광역시 통합주차정보시스템(pis.daegu.go.kr) 전용. 공공데이터포털(data.go.kr) 연동은
// 쓰지 않기로 해서 제거했다 — 이 앱은 대구시 주차장만 다룬다.
...
// 대구 구간은 실시간 혼잡도를 포함하므로 짧은 주기로 갱신한다.
...
async function getCachedLots(): Promise<ParkingLot[]> {
  ...
  if (!cache || now - cache.fetchedAt > CACHE_TTL_MS) {
    // loadDaeguCityParkingLots 내부에서 이미 키 누락/요청 실패를 전부 잡아 더미 목록으로
    // 대체하므로 사실상 던지지 않지만, 예기치 못한 예외까지 안전망으로 한 번 더 막아
    // 캐시가 깨진 상태로 남지 않게 한다.
    let lots: ParkingLot[];
    try {
      lots = await loadDaeguCityParkingLots();
    } catch (err) {
      console.warn(`[대구시 API] 예기치 못한 오류 — 임시 주차장 목록으로 대체합니다: ${(err as Error).message}`);
