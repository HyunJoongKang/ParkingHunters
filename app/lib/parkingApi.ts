...
// 여기서는 그 API가 다루지 않는 경상북도만 표준데이터에서 가져온다. 키 누락이나 요청 실패
// 시에도 전체 응답을 죽이지 않도록 대구 쪽과 같은 패턴으로 빈 배열로 대체한다 — 경북 데이터
// 없이 대구 데이터만이라도 계속 보여준다.
async function loadGyeongbukParkingLots(): Promise<ParkingLot[]> {
  ...
  if (!serviceKey) {
    console.warn(
      "[전국주차장정보표준데이터] DATA_GO_KR_PARKING_SERVICE_KEY가 설정되어 있지 않아 경북 데이터 없이 대구 데이터만 표시합니다."
    );
    return [];
  }
  ...
  try {
    ...
    console.warn(
      `[전국주차장정보표준데이터] 조회 실패 — 경북 데이터 없이 대구 데이터만 표시합니다: ${(err as Error).message}`
    );
    return [];
  }
}
...
async function getCachedLots(): Promise<ParkingLot[]> {
  ...
  if (!cache || now - cache.fetchedAt > CACHE_TTL_MS) {
    // loadDaeguCityParkingLots/loadGyeongbukParkingLots 둘 다 내부적으로 실패를 이미
    // 흡수하므로(대구는 더미 목록, 경북은 빈 배열) 사실상 던지지 않지만, 예기치 못한
    // 예외까지 안전망으로 한 번 더 막아 캐시가 깨진 상태로 남지 않게 한다.
    let lots: ParkingLot[];
    try {
      ...
      lots = [...daeguCityLots, ...gyeongbukLots];
    } catch (err) {
      console.warn(`[주차장 API] 예기치 못한 오류 — 임시 주차장 목록으로 대체합니다: ${(err as Error).message}`);
      lots = DUMMY_DAEGU_PARKING_LOTS;
    }
